...
def extract_bboxes(mask):
    """(** 5x Faster verion than original implement **) Compute bounding boxes from masks 
    mask: [height, width, num_instances]. Mask pixels are either 1 or 0.
    Returns: bbox array [num_instances, (y1, x1, y2, x2)].
    """
    mask_shape = mask.shape
    boxes = np.zeros([mask_shape[-1], 4], dtype=np.int32)
    ...
    vertical_end = mask_shape[0] - np.argmax(np.any(np.flip(mask, axis=0), axis=1), axis=0)

    # make the (y1, x1, y2, x2) bbox array
    for i, x in enumerate(zip(vertical_start, horizon_start, vertical_end, horizon_end)):
        boxes[i] = np.array(x)
...
def minimize_mask(bbox, mask, mini_shape):
    ...
    for i in range(mask.shape[-1]):
        ...
        # Don't astype if dtype is already boolean for accelerating
        if mask.dtype != "bool":
